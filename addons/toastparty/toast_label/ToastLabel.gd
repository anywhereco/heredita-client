...
func _clean_config(config: Dictionary) -> Dictionary:
	...
	if not _config.has("text_size"):
		_config["text_size"] = 16
	if not _config.has("bgcolor"):
		_config["bgcolor"] = Color(0, 0, 0, .35)
...
func move_to(index: int) -> void:
	...
	if index == 0:
		...
		(
			_tween_in
			. tween_property(self, "position", Vector2(position.x, _y), .4)
		...
		(
			_tween_in
			. tween_property(self, "position", Vector2(position.x, _y), .35)
			. set_trans(Tween.TRANS_QUAD)
			. set_ease(Tween.EASE_IN_OUT)
		)
		_tween_in.play()
