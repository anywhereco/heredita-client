...
func register():
	if is_web:
		...
		interface.register_dialog(
			get_instance_id(), FileMode.keys()[file_mode], ",".join(filters), _callback
		)
...
func show():
	...
	assert(
		is_node_ready(),
		"HTML5FileDialog cannot be shown before its _ready! did you forget to call add_child()?"
	)
	assert(
		interface != null,
		"HTML5FileDialog JS interface is null! This is probably a bug, please report it at https://gitlab.com/mocchapi/godot-4-html5-file-dialogs/-/issues"
	)
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if OS.get_name() != "Web":
		(
			out
			. append(
				"This node only works on web exports! Calling its functions in a regular build will result in failed assertions"
			)
		)
