## Handles lower-level InfernoSocket connections.
@abstract
class_name InfernoSocketClient
extends Node

var return_handshake := {
	"infernosocket_version": [4, 0],
...
var chunk_sender: InfernoSocketUtil.ChunkSender
var chunk_reciever: InfernoSocketUtil.ChunkReceiver
...
func _init(
	...
	chunk_sender = InfernoSocketUtil.ChunkSender.new(func(data: PackedByteArray) -> void: socket.send(data))
	chunk_reciever = InfernoSocketUtil.ChunkReceiver.new()
...
@abstract func _create_room() -> void: pass
#endregion
...
func _process(delta: float) -> void:
	poll()
	_ping_timer += delta
	if _ping_timer > PING_INTERVAL and socket.get_ready_state() != socket.STATE_CLOSED:
