...
func _get_minimum_size() -> Vector2:
	...
	var x := (
		(
			Left.get_combined_minimum_size()
			+ Bump.get_combined_minimum_size()
			+ Right.get_combined_minimum_size()
		)
		. x
	)
...
func _notification(what: int) -> void:
	match what:
		NOTIFICATION_DRAW:
			...
			var bump_size := (
				Bump.get_combined_minimum_size()
				+ Vector2(bump_space, -size.y + bump_protrusion + bump_bottom_margin)
			)
		...
		NOTIFICATION_SORT_CHILDREN:
			...
			var bump_size := (
				Bump.get_combined_minimum_size() + Vector2(bump_space, bump_protrusion - size.y)
			)
			fit_child_in_rect(
				Bump,
				Rect2(
					offset.x + fit_size.x / 2 - bump_size.x / 2, offset.y, bump_size.x, bump_size.y
				)
			)
			fit_child_in_rect(
