...
static func from_json_safe(json: Dictionary) -> Calendar:
	if (
		typeof(json["minutes_per_year"]) != TYPE_FLOAT
		or typeof(json["year"]) not in [TYPE_FLOAT, TYPE_INT]
		or typeof(json["month"]) not in [TYPE_FLOAT, TYPE_INT]
		or typeof(json["day"]) not in [TYPE_FLOAT, TYPE_INT]
