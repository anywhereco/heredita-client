...
func dice_events(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if State.client:
			State.client.send(
				"dice", {"position": ISUtil.from_vec3(position), "settings": settings}
			)
			cooldown = COOLDOWN
		else:
			display_result(
				MapperRoot._instance.get_node("LocalPlayer").position as Vector3, -2, roll(settings)
			)
...
func message_received(event: String, player_id: int, details: Variant) -> void:
	if event == "dice_result" and player_id == -1:
		...
		display_result(
			ISUtil.to_vec3(details["position"]),
			details["player_id"],
			DiceResult.from_data(details["result"] as Dictionary)
		)
...
static func roll(settings: Dictionary) -> DiceResult:  #only used by SERVER if not in singleplayer
	...
	return DiceResult.new(
		randi_range(settings.get("min", 1) as int, settings.get("max", 20) as int), die_string
	)
